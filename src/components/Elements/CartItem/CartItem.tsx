  ...
  return (
    <article className="  w-full border border-secondary mx-auto font-AlbertSans">
      <div className="font-bold py-2 my-auto w-full border-r border-r-secondary flex justify-between ">
        ...
        <h2 className=" text-md mr-4 dark:text-white">${price}</h2>
      </div>
      <img
        src={getImage(name)}
        alt={name}
      ...
      />
      <div className="flex w-full justify-between border border-secondary py-2 dark:bg-tertiary dark:text-primary">
        <div className="flex ml-4">
          <Button
            handleClick={() => toggleCartItemQuantity(name, "decrement")}
            className="px-2 font-extrabold   sm:px-3 sm:py-1 "
          >
            -
          </Button>
          <p
            className="border-2 px-2 border-primary rounded-md font-medium 
               dark:border-primary sm:px-3 sm:py-1"
